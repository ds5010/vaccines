...
def file_to_df_jhu(filename,col,desired_date,enddate):
    ...
    date_list = pd.date_range(start ='5-1-2021', end ='6-05-2021').strftime('%m-%d-%Y').tolist()  
    
    new_df = orig_df[orig_df['Date'].isin(date_list)][['FIPS', col]].copy()
    ...
    df_clean = df_clean.groupby("FIPS")[col].sum()
...
cdc = file_to_df_cdc('COVID-19_Vaccinations_in_the_United_States_County.csv.gz','Series_Complete_18PlusPop_Pct','05/01/2021','Census2019_18PlusPop')
jhu = file_to_df_jhu('JHU_masterfile_with_dates.csv.gz','Deaths','05-01-2021','06-01-2021')
...
jhu = file_to_df_jhu('JHU_masterfile_with_dates.csv.gz','Deaths','08-01-2021','09-01-2021')
#merging the two DFs using FIPS as the common key
merged_df = merged_data(cdc,jhu,'FIPS')
#generating a scatterplot
my_scatter(merged_df,'Series_Complete_18PlusPop_Pct','Deaths','Census2019_18PlusPop')
...
jhu = file_to_df_jhu('JHU_masterfile_with_dates.csv.gz','Deaths','11-01-2021','11-30-2021')
#merging the two DFs using FIPS as the common key
merged_df = merged_data(cdc,jhu,'FIPS')
#generating a scatterplot
my_scatter(merged_df,'Series_Complete_18PlusPop_Pct','Deaths','Census2019_18PlusPop')